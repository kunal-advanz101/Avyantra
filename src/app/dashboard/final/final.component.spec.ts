...
describe('FinalComponent', () => {
  ...
  it('finalFormSubmit method', () => {
    ...
    babyFinalForm.value["days_of_stay_in_hospital"] = '';
    babyFinalForm.value["final_diagnosis_other"] = '';
    component.finalFormSubmit();
    expect(babyFinalForm.value["days_of_stay_in_hospital"]).toBe('NA');
    expect(babyFinalForm.value["final_diagnosis_other"]).toBe('NA');
  });

  it('success method', () => {
    component.page = 1;

    component.success({}, "get_final");
    expect(component.responseArray.length).toBe(0);

    component.success({ response: [1,2,3], status:200 }, "get_final");
    expect(component.responseArray.length).toBe(3);
  });
