...
export class MaternalComponent implements OnInit, OnChanges {
  maternalForm: FormGroup; formRef: any; submitted = false; already_exist_status = 422; success_status = 200; page: number = 1; is_api_call = true;
  getMedicalRecordNumber: string; chkMotherAge: boolean = true; chkMotherWeight: boolean = true; chkMotherHeight: boolean = true; chkFeverUnit: boolean = true;
  chkThyroidUnit : boolean = true; chkMotherBmi: boolean = true; chkMotherHaemoglobin: boolean = true; chkMaternalBPSys: boolean = true;
  chkMaternalBPDias: boolean = true; chkRuptureIfProm: boolean = true; chkAminoticFluidCultureIfPos: boolean = true; chkVagSwabCulture: boolean = true;
  isBoolMotherHeight: boolean = true; updateFlag: boolean = false; isMotherEdit: boolean = false; localObj:any;isEditable=true;
  @Input() id;
  @Input() hospital_id;
  subscription: Subscription;
  temp_study_id = 0; login_hospital: any = {}; responseArray = [];
  public dataServiceObj; public babyReadingData;
  constructor(
    private formBuilder: FormBuilder,private router: Router,private toastr: ToastrService,private common_api: CommonService,private modalService: NgbModal,private commonAsyn: Common,private dataService:DataService, public readingDataService:ReadingDataService
  ...
  createForm(id) {
    ...
    vim.maternalForm = vim.formBuilder.group({
      study_id: [vim.id],  mother_age: ["", [Validators.required]],  mother_weight_unit: ["Kgs", Validators.required], mother_weight: ["", Validators.required],
      mother_height: ["", Validators.required],mother_height_unit: ["ft", Validators.required],mother_haemoglobin: ["", Validators.required],
      mother_bmi: ["",Validators.required], maternal_blood_pressure: ["", Validators.required],
      maternal_blood_pressure_diastolic: ["", Validators.required],maternal_diabetes: ["", Validators.required], maternal_fever: ["", Validators.required],
      maternal_fever_unit: ["Centigrade", Validators.required],
      maternal_fever_basic: ["", Validators.required], maternal_thyroid_function: ["", Validators.required], maternal_thyroid_function_basic: ["", Validators.required],
      maternal_thyroid_function_unit_basic: ["", Validators.required], maternal_thyroid_function_unit_basic_unit: ["mU/L", Validators.required],
      more_than_3_vaginal_examinations_during_labor: ["", Validators.required],rupture_of_membranes_rom_two: ["", Validators.required],rupture_of_membranes_rom_one: ["", Validators.required],
  ...
  calculateBMI() {
    ...
    if(this.maternalForm.value["mother_height"].length >0 && this.maternalForm.value["mother_weight"].length >0) {
      this.maternalForm.patchValue({  mother_bmi: calculatedBMI})
    } else {
      this.maternalForm.patchValue({ mother_bmi: 0})
    }
  }
  ...
  petchUpdate(val, key) {
    ...
    if (!number.test(val)) {
      ...
      if ( vim.maternalForm.value["mother_height"] != "" &&  vim.maternalForm.value["mother_weight"] != "" ) {
        const squareOfHieght = vim.maternalForm.value["mother_height"] * vim.maternalForm.value["mother_height"];
  ...
  updateForm(obj) {
    ...
    vim.maternalForm.patchValue({
      ...
      maternal_blood_pressure_diastolic: obj["maternal_blood_pressure_diastolic"],
      ...
      maternal_thyroid_function_unit_basic_unit:obj["maternal_thyroid_function_unit_basic_unit"],
      more_than_3_vaginal_examinations_during_labor:obj["more_than_3_vaginal_examinations_during_labor"],
      ...
      colonisation_or_urinary_tract_infection:obj["colonisation_or_urinary_tract_infection"],
  ...
  success(response, api_type) {
    ...
    if (api_type == "MaternalFormSubmit") {
      if (vim.isSuccess(response)) {
        ...
        if(response["status"] == 200){
          vim.toastr.success( "", "Data Inserted Succesfully" );
          vim.is_api_call = true;
         vim.dataServiceObj = vim.dataService.getOption();
         vim.get_maternal(vim.dataServiceObj.study_id, vim.login_hospital['id'], vim.page);
        }
  ...
  update_maternal_form() {
    ...
    if(vim.maternalForm.invalid) {  return;
      ...
      this.setFormData()
    vim.common_api.updateMaternalProfile(vim.id, vim.maternalForm.value).subscribe(result => {
